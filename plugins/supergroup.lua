...
local title =      "🌐 معلومات المجموعه خارقه👋😈:\n ❗️"..result.title.."❕\n" 
local admin_num =  "🔰عدد الادمنية: "..result.admins_count.."\n" 
local user_num =   "🚸عدد الاشخاص: "..result.participants_count.."\n" 
local kicked_num = "🚨 عدد الاشخاص المطرودين: "..result.kicked_count.."🏌\n" 
...
local function run(msg, matches) 
   ...
   local data = load_data(_config.moderation.data) 
      if matches[1] == 'add' and not matches[2] then 
        ...
        local run = "is already added !" 
         return send_large_msg(receiver, run) 
         end 
      ...
      if matches[1] == 'add' and is_admin1(msg) and not matches[2] then 
         if not is_super_group(msg) then 
           local run = "This group not added 😳 OK I will add this group" 
            return send_large_msg(receiver, run) 
         end 
      ...
      if matches[1] == 'silent' and is_owner(msg) then 
         local chat_id = msg.to.id 
         ...
         local msg_type = 'Audio' 
            if not is_muted(chat_id, msg_type..': yes') then 
               ...
               return "SuperGroup #Silent "..msg_type.." is already on👿✋" 
            end 
         end 
         ...
         local msg_type = 'All' 
            if not is_muted(chat_id, msg_type..': yes') then 
               savelog(msg.to.id, name_log.." ["..msg.from.id.."] set SuperGroup to: mute "..msg_type) 
               mute(chat_id, msg_type) 
               return "#Silent "..msg_type.."  has been enabled please dont talk🔕" 
            else 
               return "#Silent "..msg_type.." is already on👿✋" 
            end 
         end 
      ...
      if matches[1] == 'unsilent' and is_momod(msg) then 
         ...
         local msg_type = 'Audio' 
            if is_muted(chat_id, msg_type..': yes') then 
               ...
               return "#Silent "..msg_type.." is already off👿✋" 
            end 
         end 
         if matches[2] == 'photo' then 
         local msg_type = 'Photo' 
         ...
         local msg_type = 'Text' 
            if is_muted(chat_id, msg_type..': yes') then 
               ...
               return "#Silent text is already of👿✋" 
            end 
         end 
         if matches[2] == 'all' then 
         local msg_type = 'All' 
            if is_muted(chat_id, msg_type..': yes') then 
               savelog(msg.to.id, name_log.." ["..msg.from.id.."] set SuperGroup to: unmute "..msg_type) 
               unmute(chat_id, msg_type) 
               return "#Silent "..msg_type.." has been disabled you can talk now👋" 
            else 
               return "#Silent "..msg_type.." is already disabled" 
            end 
         end 
      end 
